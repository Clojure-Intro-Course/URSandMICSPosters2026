...
%One focus of our research is development of a tool called Babel to modify Clojure error messages [3,4].
        \item To address the error messages issues, Babel provides a way to directly replace error messages in the REPL with simplified, beginner-friendly versions.
...
\emcomment{Add some explanation of how it's related to error messages work}
 ...
 \block{Examples \emcomment{Make the titles more concrete}}{
...
Notice how with the check-equal and check-range functions, I've recreated the format of the internal function call, to show students what is really happening. \\
    ...
    \item The function \texttt{(draw-state [state])}, which prints a representation of the game state to the console.
    \item The function \texttt{(win? [state player])}, which returns true if the given player has won the game, and false otherwise. The framework will skip the second player if there is none, as students have the choice to create one-player or two-player games.
    \end{itemize}
